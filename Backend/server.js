...
//   cors: {
//     origin: ["https://localhost:5173", "http://localhost:3000", "https://stark-socialmedia.vercel.app"],
//     methods: ["GET", "POST"],
...
const allowedOrigins = [
  "http://localhost:5173",
  "http://localhost:3000",
  "https://stark-socialmedia.vercel.app"
];
...
io.on("connection", (socket) => {
  ...
  socket.on(
    "sendMessage",
    async ({ senderId, receiverId, content, media, messageType, postId }) => {
      try {
        const isReceiverOnline = activeUsers.has(receiverId);
  ...
  socket.on(
    "incomingCall",
    ({ callerId, receiverId, callType, callerName, callerProfilePic, roomID }) => {
      const receiverSocket = activeUsers.get(receiverId);
      if (receiverSocket) {
        io.to(receiverSocket).emit("incomingCall", {
          callerId,
