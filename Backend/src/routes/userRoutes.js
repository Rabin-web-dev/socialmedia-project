...
router.post("/unfollow/:username/:userId", authMiddleware, async (req, res) => {
  ...
  const currentUserId = req.user.id;
  try {
    if (currentUserId === userId) return res.status(400).json({ message: "You can't unfollow yourself" });
    ...
    userToUnfollow.followers = userToUnfollow.followers.filter(f => f.toString() !== currentUserId);
    currentUser.following = currentUser.following.filter(f => f.toString() !== userId);
